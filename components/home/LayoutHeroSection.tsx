import { Badge } from "@/components/ui/badge";
import { Button } from "@/components/ui/button";
import { ArrowRight } from "lucide-react";
import Image from "next/image";
import Link from "next/link";
import { homeContent } from "@/content/home";

const { hero } = homeContent;

export const LayoutHeroSection = () => {
  return (
    <section className="container w-full">
      <div className="grid place-items-center lg:max-w-screen-xl gap-8 mx-auto py-20 md:py-32">
        <div className="text-center space-y-8">
          <Badge variant="outline" className="text-sm py-2 badge-green-gradient">
            <span className="mr-2 text-primary">
              <Badge variant="default" className="btn-green-glow">
                {hero.badgeInner}
              </Badge>
            </span>
            <span>{hero.badgeOuter}</span>
          </Badge>

          <div className="max-w-screen-md mx-auto text-center text-4xl md:text-6xl font-bold">
            <h1>
              {hero.titleBefore}
              <span className="text-transparent px-2 bg-gradient-to-r from-green-400 via-emerald-500 to-primary bg-clip-text">
                {hero.titleHighlight}
              </span>
              {hero.titleAfter}
            </h1>
          </div>

          <p className="max-w-screen-sm mx-auto text-xl text-muted-foreground">
            {hero.subtitle}
          </p>

          <div className="space-y-4 md:space-y-0 md:space-x-4 flex flex-col md:flex-row justify-center">
            <Button asChild className="w-5/6 md:w-1/4 font-bold group/arrow btn-green-glow shadow-lg">
              <Link href={hero.primaryCta.href}>
                {hero.primaryCta.label}
                <ArrowRight className="size-5 ml-2 group-hover/arrow:translate-x-1 transition-transform" />
              </Link>
            </Button>

            <Button
              asChild
              variant="secondary"
              className="w-5/6 md:w-1/4 font-bold border border-primary text-primary hover:bg-primary/10"
            >
              <Link href={hero.secondaryCta.href}>{hero.secondaryCta.label}</Link>
            </Button>
          </div>
        </div>

        <div className="relative group mt-14">
          <div className="absolute top-2 lg:-top-8 left-1/2 transform -translate-x-1/2 w-[90%] mx-auto h-24 lg:h-80 bg-primary/30 rounded-full blur-3xl"></div>
          <Image
            width={1200}
            height={1200}
            className="w-full md:w-[1200px] mx-auto rounded-lg relative leading-none flex items-center border border-t-2 border-secondary border-t-primary/30 dark:hidden"
            src={hero.heroImageLight}
            alt={hero.heroImageAlt}
            priority
          />
          <Image
            width={1200}
            height={1200}
            className="hidden w-full md:w-[1200px] mx-auto rounded-lg relative leading-none border border-t-2 border-secondary border-t-primary/30 dark:block"
            src={hero.heroImageDark}
            alt={hero.heroImageAlt}
            priority
          />
          <div className="absolute bottom-0 left-0 w-full h-20 md:h-28 bg-gradient-to-b from-background/0 via-background/50 to-background rounded-lg"></div>
        </div>
      </div>
    </section>
  );
};